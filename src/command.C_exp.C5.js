                                ...
                                case 22:
                                    ...
                                    if (ev.xconfigure.window == parent[0]){
                                        while (XCheckTypedWindowEvent (dpy, ev.xconfigure.window, 22, &ev))
                                            ;
                                        if (szHint.width != ev.xconfigure.width || szHint.height != ev.xconfigure.height){ //                  if (szHint.width != ev.xconfigure.width || szHint.height != ev.xconfigure.height){
                                            seen_resize = 1;
                                        ...
                                        rxvt_term.next_ =function(){
                                            while (cmdbuf_ptr < cmdbuf_endp){
                                                ...
                                                if (__builtin_expect (((( ) c mdbuf_ptr <= 0x7f &&  c mdbuf_ptr != 0x1b) != 0),(1)))
                                                    return  cmdbuf_ptr++; //CMNT: possible_pointer ^|               return *cmdbuf_ptr++;


                                                var   wc; //             wchar_t wc;
                                                var len = mbrtowc (&wc, cmdbuf_ptr, cmdbuf_endp - cmdbuf_ptr, mbstate); // FIXME pointer     size_t len = mbrtowc (&wc, cmdbuf_ptr, cmdbuf_endp - cmdbuf_ptr, mbstate);
...
/*{{{ process escape sequences */
    ...
    rxvt_term.process_escape_seq =function(){
        ...
        case C1_NEL: /* ESC E */
                {
                    var nlcr= [ C0_LF, C0_CR ] ; //FIXME                wchar_t nlcr[] = { C0_LF, C0_CR };
...
//FIXME I already wrote this in another file
var enum2 = {
    CSI_ICH = 0x40,
        ...
        case URxvt_font:
            ...
            if (query)
                ...
                //CMNT: c_keyword possible_pointer ^|                   const char *&res = rs[Rs_font + (op - URxvt_font)];
                    & res = rs[Rs_font + (op - URxvt_font)]; //FIXME

                res = strdup (str);
                //CMNT: c_keyword possible_pointer ^|                   allocated.push_back ((void *)res);
                allocated.push_back ((  ) res);  //FIXME
                set_fonts ();
            }
            break;
